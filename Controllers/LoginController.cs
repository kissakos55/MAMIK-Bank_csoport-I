    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSalt(string felhasznaloNev)
        {
            ...
            using (var cx = new MamikBankContext())
            {
                try
                {
                    Felhasznalok response = await cx.Felhasznaloks.FirstOrDefaultAsync(f => f.FelhasznaloNev == felhasznaloNev);
        ...
        public async Task<IActionResult> Login(LoginDTO loginDTO)
        {
            using (var cx = new MamikBankContext())
            {
                try
                {
                    ...
                    Felhasznalok loggedUser = await cx.Felhasznaloks.FirstOrDefaultAsync(f => f.FelhasznaloNev == loginDTO.LoginName && f.Hash == Hash);
                    if (loggedUser != null && loggedUser.Aktiv == 1)
                    {
                        ...
                        return Ok(new LoggedUser { Name = loggedUser.TeljesNev, Email = loggedUser.Email, Permission = loggedUser.Jogosultsag, ProfilePicturePath = loggedUser.FenykepUtvonal, Token = token , UgyfelAzonosito=loggedUser.Id});
                    }
                    else
                    {
